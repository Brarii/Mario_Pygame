...
class Dragon:
    ...
    def update(self):
        ...
        if self.dragon_img_rect.top <= cactus_img_rect.bottom:
            game_over2()
        if self.dragon_img_rect.bottom >= fire_img_rect.top:
            game_over2()
        if keys[pygame.K_UP]:
            self.dragon_img_rect.top -= self.velocity
        elif keys[pygame.K_DOWN]:
            self.dragon_img_rect.top += self.velocity

        if self.dragon_img_rect.top <= cactus_img_rect.bottom:
            self.dragon_img_rect.top = cactus_img_rect.bottom
        elif self.dragon_img_rect.bottom >= fire_img_rect.top:
            self.dragon_img_rect.bottom = fire_img_rect.top
...
class Flames:
    ...
    def __init__(self):
        ...
        self.flames_img_rect.left = mario.mario_img_rect.right
        self.flames_img_rect.top = mario.mario_img_rect.top + 50

    def update(self):
        canvas.blit(self.flames_img, self.flames_img_rect)

        if self.flames_img_rect.left < WINDOW_WIDTH:
...
def game_over2():
    ...
    music.play()
    topscore.top_score(SCORE)
    game_over_img = pygame.image.load('endm.jpg')
    game_over_img_rect = game_over_img.get_rect()
    game_over_img_rect.center = (WINDOW_WIDTH/2, WINDOW_HEIGHT/2)
    canvas.blit(game_over_img, game_over_img_rect)
    while True:
    ...
    while True:
        ...
        while True:
            ...
            for f in flames_list:
                if f.flames_img_rect.right <= 0:
                    flames_list.remove(f)
                    SCORE += 1
                f.update()
                flames_list2.append(new_flame2)
                for n in flames_list2:
                    if n.flames_img_rect.right <= 0:
                        flames_list.remove(n)
                        SCORE += 1
                    n.update()
            ...
            for f in flames_list:
                if f.flames_img_rect.colliderect(mario.mario_img_rect):
                    game_over()
                if f.flames_img_rect.colliderect(dragon.dragon_img_rect):
                    game_over2()
